...
def rolling_form(tg: pd.DataFrame, roll: int = ROLL, minp: int = MINP) -> pd.DataFrame:
    ...
    tg = tg.copy()
    tg["off_r10"] = tg.groupby("team")["pts_for"]      .transform(lambda s: s.shift().rolling(roll, min_periods=minp).mean())
    tg["def_r10"] = tg.groupby("team")["pts_against"]  .transform(lambda s: s.shift().rolling(roll, min_periods=minp).mean())
    return tg

def join_matchups(g: pd.DataFrame, tg: pd.DataFrame) -> pd.DataFrame:
    """Join team-game features back onto the original matchup rows."""
    h = tg.rename(columns={
        "team":"home_team", "off_r10":"home_off_r10", "def_r10":"home_def_r10", "rest_days":"home_rest"
    })[["GAME_DATE","home_team","home_off_r10","home_def_r10","home_rest"]]

    a = tg.rename(columns={
        "team":"away_team", "off_r10":"away_off_r10", "def_r10":"away_def_r10", "rest_days":"away_rest"
